...
import "pkg:/source/roku_modules/maestro/core/Types-Polyfill.bs"
...
function updateChildren(newChildren = invalid)
  ...
  for each child in newChildren
    if m.protectedChildIds[child.id] = invalid
      ...
      if child.getChild(1) <> invalid
        if child.getChild(1).id.startsWith("markupGrid")
          child.getChild(1).observeFieldScoped("currFocusRow", "onMarkupChange")
        end if
      end if

      if child.id.startsWith("repeater")
        child.observeFieldScoped("currFocusRow", "onRepeaterChildrenChange")
      end if
...
function onRepeaterChildrenChange()
  onNeedsRecalculatingChange()
end function
...
function setClippingRectangle()
  ...
  m.container_moveableContent.clippingRect = [
    clippingOffsets[0]
    clippingOffsets[1]
    m.top.size[0] + clippingOffsets[2]
    m.top.size[1] + clippingOffsets[3]
  ]
end function
...
function moveContentTo(previousY, newY, forceNoAnimation = false)
  if m.top.isAnimated and not forceNoAnimation
    ...
    m.container_interpolator.keyValue = [
      [
        0
        m.previousY
      ]
    ...
    m.container_layoutGroup.translation = [
      0
      newY
    ]
  end if
...
function getYOfIndex(index as integer)
  ...
  for i = 0 to index - 1
    ...
    gridChild = child.getChild(1)
    if child.id.startsWith("repeater")
      y += calculateHeight(child, y)
    else if gridChild <> invalid and gridChild.id.startsWith("markupGrid") and gridChild.vertFocusAnimationStyle = "fixedfocus"
      y += calculateGridHeight(gridChild, y)
...
function onNeedsRecalculatingChange()
  ...
  for each child in m.container_layoutGroup.getChildren(-1, 0)
    ...
    if child.id.startsWith("repeater")
      ...
      y += calculateGridHeight(gridChild, y)
    else if child.containerHeightOverride <> invalid and child.containerHeightOverride <> 0
      y += child.containerHeightOverride
    else
      y += child.boundingRect().height
    end if

    mc_setOrAddNodeField(child, "heroControl", heroControl)
    mc_setOrAddNodeField(child, "isHeroOffsetApplied", child.isHeroOffsetApplied = true)
    mc_setOrAddNodeField(child, "_multiContainer", m.top)
...
function calculateGridHeight(child, y)
  numberOfRows = child.numRows

  if child.itemSize[1] <> invalid
    height = child.itemSize[1] + child.itemSpacing[1] + child.translation[1]
    return height * (numberOfRows - child.currFocusRow)
  end if
  return 0
end function

function calculateHeight(child, y)
  numberOfRows = child.horizontalLists.count()
  childRowlist = child.rowlist
  index = numberOfRows - 1
  if index < 0
    index = 0
  end if
  if childRowlist.rowHeights[index] <> invalid
    height = childRowlist.rowHeights[index] + childRowlist.rowLabelOffset[index][1] + childRowlist.itemSpacing[1]
    return height * (numberOfRows - childRowlist.currFocusRow)
  end if
  return 0
end function
...
function m_createNodeVars()
  for each id in [
      "container_updateTimer"
      "container_backgroundGroup"
      "container_moveableContent"
      "container_layoutGroup"
      "container_animation"
      "container_interpolator"
    ]
